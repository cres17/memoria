...
class CurveData {
  ...
  List<int> toLut() {
    ...
    if (n == 1) return List.filled(256, (sorted[0].y * 255).round().clamp(0, 255));
    if (n == 2) {
      // 선형 보간
      return List.generate(256, (i) {
        final t = i / 255.0;
        final y = sorted[0].y + (sorted[1].y - sorted[0].y) *
            ((t - sorted[0].x) / (sorted[1].x - sorted[0].x)).clamp(0.0, 1.0);
  ...
  CurveData copyWith({List<CurvePoint>? points, CurveChannel? channel}) =>
      CurveData(points: points ?? this.points, channel: channel ?? this.channel);
  ...
  factory CurveData.fromJson(Map<String, dynamic> j) => CurveData(
    ...
    points: (j['points'] as List).map((p) => CurvePoint.fromJson(p as Map<String, dynamic>)).toList(),
  );

  String toJsonString() => jsonEncode(toJson());
  factory CurveData.fromJsonString(String s) => CurveData.fromJson(jsonDecode(s) as Map<String, dynamic>);
}
...
abstract class CurvePresets {
  ...
  static CurveData brighten(CurveChannel ch) => CurveData(
    channel: ch,
    points: const [CurvePoint(0, 0), CurvePoint(0.5, 0.65), CurvePoint(1, 1)],
  );

  static CurveData darken(CurveChannel ch) => CurveData(
    channel: ch,
    points: const [CurvePoint(0, 0), CurvePoint(0.5, 0.35), CurvePoint(1, 1)],
  );

  static CurveData faded(CurveChannel ch) => CurveData(
    channel: ch,
    points: const [CurvePoint(0, 0.07), CurvePoint(0.5, 0.5), CurvePoint(1, 0.93)],
  );

  static CurveData softContrast(CurveChannel ch) => CurveData(
    channel: ch,
    points: const [
      CurvePoint(0, 0), CurvePoint(0.25, 0.2),
      CurvePoint(0.75, 0.8), CurvePoint(1, 1),
    ],
  );
  ...
  static CurveData hardContrast(CurveChannel ch) => CurveData(
    channel: ch,
    points: const [
      CurvePoint(0, 0), CurvePoint(0.3, 0.15),
      CurvePoint(0.7, 0.85), CurvePoint(1, 1),
    ],
  );

  static const presetNames = ['Neutral', 'Brighten', 'Darken', 'Faded', 'Soft Contrast', 'Hard Contrast'];

  static CurveData fromPresetName(String name, CurveChannel ch) {
    switch (name) {
      case 'Brighten':      return brighten(ch);
      case 'Darken':        return darken(ch);
      case 'Faded':         return faded(ch);
      case 'Soft Contrast': return softContrast(ch);
      case 'Hard Contrast': return hardContrast(ch);
      default:              return neutral(ch);
    }
  }
}
