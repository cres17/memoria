...
class SegmentMask {
  ...
  static List<double> _gaussKernel(int r) {
    final sigma = r / 2.0;
    final k = List<double>.generate(2 * r + 1,
        (i) => _exp(-0.5 * ((i - r) / sigma) * ((i - r) / sigma)));
    return k;
  }
  ...
  static double _fastExp(double x) {
    ...
    if (x >= 0) return 1.0;
    final ax = -x;
    return 1.0 / (1.0 + ax + ax*ax/2 + ax*ax*ax/6 + ax*ax*ax*ax/24);
  }
}
...
class SelfieSegmenter {
  ...
  SegmentMask segment(img.Image image) {
    ...
    final output = List.generate(
      1, (_) => List.generate(_inH, (_) => List.generate(_inW, (_) => [0.0])));
...
class MulticlassSegmenter {
  ...
  SemanticMasks segment(img.Image image) {
    ...
    // Build per-class masks (argmax winner takes all, then feather)
    final classData = List.generate(
        _nClasses, (_) => Float32List(_inWH * _inWH));

    for (int y = 0; y < _inWH; y++) {
      for (int x = 0; x < _inWH; x++) {
        final scores = output[0][y][x] as List;
        for (int c = 0; c < _nClasses; c++) {
          classData[c][y * _inWH + x] = (scores[c] as double).clamp(0.0, 1.0);
        }
      }
    }
