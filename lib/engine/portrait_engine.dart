...
extension SkinToneX on SkinTone {
  String get label {
    switch (this) {
      case SkinTone.none:   return '없음';
      case SkinTone.pale:   return 'Pale';
  ...
  double? get targetHue {
    switch (this) {
      case SkinTone.none:   return null;
      case SkinTone.pale:   return 15.0;
      case SkinTone.fair:   return 20.0;
      case SkinTone.medium: return 25.0;
      case SkinTone.dark:   return 30.0;
    }
  }
}
...
img.Image applySkinSmoothing(
  ...
  final blurred = img.gaussianBlur(image, radius: (sigma * 3).round().clamp(1, 8));
  ...
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      ...
      if (maskVal < 0.01) {
        ...
        result.setPixelRgb(x, y,
          (px.rNormalized * 255).round(), (px.gNormalized * 255).round(), (px.bNormalized * 255).round());
        continue;
      }
...
img.Image applySkinToning(
  ...
  if (tone == SkinTone.none || strength <= 0) return image;
  ...
  for (int y = 0; y < image.height; y++) {
    for (int x = 0; x < image.width; x++) {
      ...
      final hsv      = _rgbToHsv(px.rNormalized.toDouble(), px.gNormalized.toDouble(), px.bNormalized.toDouble());
      ...
      result.setPixelRgb(x, y,
        (rgb.$1 * 255).round(), (rgb.$2 * 255).round(), (rgb.$3 * 255).round());
    }
  }
  return result;
}
