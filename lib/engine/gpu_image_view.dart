...
const _kHslBandOrder = [
  HslBand.red, HslBand.orange, HslBand.yellow, HslBand.green,
  HslBand.cyan, HslBand.blue, HslBand.purple, HslBand.magenta,
];
...
Future<ui.Image> buildLumCurveTexture(AdjustParams p) async {
  ...
  if (active) {
    ...
    for (int i = 0; i < 256; i++) {
      ...
      final stored = ((remapped - original + 0.5) * 255.0).round().clamp(0, 255);
      pixels[i * 4]     = stored;
      pixels[i * 4 + 1] = stored;
      pixels[i * 4 + 2] = stored;
      pixels[i * 4 + 3] = 255;
...
class _GpuImageViewState extends State<GpuImageView> {
  ...
  void didUpdateWidget(GpuImageView oldWidget) {
    ...
    // If not sliding or notifier is not active, sync from props
...
class _AdjustPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final lumCurveActive = p.luminanceCurve != null && !p.luminanceCurve!.isLinear;
...
Future<ui.Image> _make1x1(int r, int g, int b, int a) async {
  final pixels = Uint8List(4)..[0] = r..[1] = g..[2] = b..[3] = a;
  final codec = await ui.ImageDescriptor.raw(
    await ui.ImmutableBuffer.fromUint8List(pixels),
...
Future<ui.Image> _makeIdentityCurve() async {
  ...
  final codec = await ui.ImageDescriptor.raw(
    await ui.ImmutableBuffer.fromUint8List(pixels),
    width: 256, height: 1, pixelFormat: ui.PixelFormat.rgba8888,
  ).instantiateCodec();
  return (await codec.getNextFrame()).image;
}
