// The benchmark keeps compact single-line loops for scanability in reports.
...
_Lab _toL(_Rgb c) {
  ...
  return _Lab(116 * _f(y / _yn) - 16, 500 * (_f(x / _xn) - _f(y / _yn)),
      200 * (_f(y / _yn) - _f(z / _zn)));
}
...
_Rgb _toR(_Lab c) {
  ...
  return _Rgb(
      _delin(rl.clamp(0, 1)), _delin(gl.clamp(0, 1)), _delin(bl.clamp(0, 1)));
}
...
_StyleProfile _analyze(img.Image im) {
  final rH = List<int>.filled(256, 0),
      gH = List<int>.filled(256, 0),
      bH = List<int>.filled(256, 0);
  var sSumA = 0.0,
      sSumB = 0.0,
      mSumA = 0.0,
      mSumB = 0.0,
  ...
  for (int y = 0; y < im.height; y++)
    for (int x = 0; x < im.width; x++) {
      ...
      final r = px.rNormalized.toDouble(),
          g = px.gNormalized.toDouble(),
          b = px.bNormalized.toDouble();
...
double _f16(int h) {
  final e = (h >> 10) & 0x1F, m = h & 0x3FF;
  if (e == 0) return 0;
  if (e == 31) return double.infinity;
  return ((h >> 15) == 0 ? 1 : -1) * (1 + m / 1024.0) * math.pow(2, e - 15);
}
  ...
  for (int r = 0; r < dim; r++)
    for (int g = 0; g < dim; g++) {
      for (int b = 0; b < dim; b++) {
        ...
        final r1 = p.rC[r8] / 255.0,
            g1 = p.gC[g8] / 255.0,
            b1 = p.bC[b8] / 255.0;
        final lab = _toL(_Rgb(r1, g1, b1));
        double gw(double l, double c) {
          final d = l - c;
          return math.exp(-0.5 * d * d / (25.0 * 25.0));
        }

        final ws = gw(lab.l, 17.5), wm = gw(lab.l, 50.0), wh = gw(lab.l, 82.5);
        final wT = ws + wm + wh + 1e-10;
        const ts = 0.15;
        final aO = (lab.a + ts * (ws * p.s.a + wm * p.m.a + wh * p.h.a) / wT)
            .clamp(-110.0, 110.0);
        final bO = (lab.b + ts * (ws * p.s.b + wm * p.m.b + wh * p.h.b) / wT)
            .clamp(-110.0, 110.0);
        final o = _toR(_Lab(lab.l, aO, bO));
        lut[idx++] = _h16(o.r.clamp(0, 1));
        lut[idx++] = _h16(o.g.clamp(0, 1));
        lut[idx++] = _h16(o.b.clamp(0, 1));
      }
    }
...
Uint8List _lutOld(img.Image style) {
  ...
  final muL = lv.fold(0.0, (s, v) => s + v) / n,
      muA = av.fold(0.0, (s, v) => s + v) / n,
      muB = bv.fold(0.0, (s, v) => s + v) / n;
  final sigL = math
      .sqrt(lv.fold(0.0, (s, v) {
            final d = v - muL;
            return s + d * d;
          }) /
          n)
  ...
  final sigA = math
      ...
      .clamp(0.001, 99.0);
  final sigB = math
      .sqrt(bv.fold(0.0, (s, v) {
            final d = v - muB;
            return s + d * d;
          }) /
          n)
      .clamp(0.001, 99.0);
  final tot = lH.fold(0, (a, b) => a + b);
  double c = 0;
  ...
  final sLab = sC > 0
      ? _Lab(sSumL / sC, sSumA / sC, sSumB / sC)
      : const _Lab(17.5, 0, 0);
  final mLab =
      mC > 0 ? _Lab(mSumL / mC, mSumA / mC, mSumB / mC) : _Lab(50, muA, muB);
  final hLab = hC > 0
      ? _Lab(hSumL / hC, hSumA / hC, hSumB / hC)
      : const _Lab(82.5, 0, 0);
  const dim = 33;
    ...
    for (int g = 0; g < dim; g++) {
      for (int b = 0; b < dim; b++) {
        ...
        double gw(double l, double center) {
          final d = l - center;
          return math.exp(-0.5 * d * d / 484);
        }

        final ws = gw(lOut, sLab.l),
            wm = gw(lOut, mLab.l),
            wh = gw(lOut, hLab.l);
        final wT = ws + wm + wh + 1e-10;
...
_Rgb _applyLut(Uint8List bytes, _Rgb c) {
  ...
  _Rgb s(int r, int g, int b) {
    final i = (r + g * dim + b * dim * dim) * 3;
    return _Rgb(_f16(lut[i]), _f16(lut[i + 1]), _f16(lut[i + 2]));
  }

  _Rgb lerp(_Rgb a, _Rgb b, double t) =>
...
img.Image _applyLutImg(img.Image src, Uint8List lutBytes) {
  ...
  for (int y = 0; y < src.height; y++)
    for (int x = 0; x < src.width; x++) {
      ...
      final o = _applyLut(
          lutBytes,
          _Rgb(px.rNormalized.toDouble(), px.gNormalized.toDouble(),
              px.bNormalized.toDouble()));
      out.setPixelRgb(x, y, (o.r.clamp(0, 1) * 255).round(),
          (o.g.clamp(0, 1) * 255).round(), (o.b.clamp(0, 1) * 255).round());
    }
  return out;
}
...
img.Image _makeGradientTest() {
  ...
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      ...
      final f = h6 - i;
      const sat = 0.8;
      final p = brightness * (1 - sat),
          q = brightness * (1 - sat * f),
          t2 = brightness * (1 - sat * (1 - f));
      final bv = brightness;
      double r, g, b;
      switch (i % 6) {
        case 0:
...
img.Image _makeFortressStyle() {
  ...
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      final ny = (y / h);
      final brightness =
          0.1 + ny * 0.85 + rng.nextDouble() * 0.05; // bottom=bright(sky)
      ...
      im.setPixelRgb(x, y, (sc(r) * 255).toInt(), (sc(g) * 255).toInt(),
          (sc(b) * 255).toInt());
    }
  return im;
}
...
img.Image _makeTramStyle() {
  ...
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      final brightness =
          (y / h * 0.8 + 0.1 + rng.nextDouble() * 0.05).clamp(0.0, 1.0);
      double r, g, b;
      if (brightness < 0.4) {
...
img.Image _makeMatteFadeStyle() {
  ...
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) {
      final brightness =
          (y / h * 0.6 + 0.1 + rng.nextDouble() * 0.04).clamp(0.0, 1.0);
...
img.Image _strip(String label, img.Image orig, img.Image resN, img.Image resO,
    img.Image style) {
  const labelW = 90, gap = 4, textH = 18;
  final h = orig.height + textH;
  final w =
      labelW + orig.width + resN.width + resO.width + style.width + gap * 5;
  final strip = img.Image(
      width: w, height: h, backgroundColor: img.ColorRgb8(20, 20, 20));
  ...
  void paste(img.Image dst, img.Image src, int ox, int oy) {
    for (int y = 0; y < src.height; y++)
      for (int x = 0; x < src.width; x++) {
        final px = src.getPixel(x, y);
        dst.setPixelRgb(
            ox + x, oy + y, px.r.toInt(), px.g.toInt(), px.b.toInt());
      }
  }

  // Draw vertical label block on left
  img.fillRect(strip,
      x1: 0, y1: 0, x2: labelW, y2: h, color: img.ColorRgb8(30, 30, 30));
  try {
    img.drawString(strip, label,
        font: img.arial14,
        x: 4,
        y: h ~/ 2 - 7,
        color: img.ColorRgb8(255, 200, 80));
  } catch (_) {}

  int ox = labelW + gap;
  drawLabel(strip, ox, 0, 'ORIGINAL', orig.width);
  paste(strip, orig, ox, textH);
  ox += orig.width + gap;
  drawLabel(strip, ox, 0, 'NEW', resN.width);
  paste(strip, resN, ox, textH);
  ox += resN.width + gap;
  drawLabel(strip, ox, 0, 'OLD', resO.width);
  paste(strip, resO, ox, textH);
  ox += resO.width + gap;
  drawLabel(strip, ox, 0, 'STYLE SRC', style.width);
  paste(strip, style, ox, textH);
...
(double, double) _deltaE(img.Image a, img.Image b) {
  ...
  for (int y = 0; y < a.height; y++)
    for (int x = 0; x < a.width; x++) {
      ...
      final la = _toL(_Rgb(ap.rNormalized.toDouble(), ap.gNormalized.toDouble(),
          ap.bNormalized.toDouble()));
      final lb = _toL(_Rgb(bp.rNormalized.toDouble(), bp.gNormalized.toDouble(),
...
void main(List<String> args) {
  ...
  for (final entry in styles.entries) {
    ...
    print(
        '  Shadow  cast: a=${profile.s.a.toStringAsFixed(1)} b=${profile.s.b.toStringAsFixed(1)}');
    print(
        '  Midtone cast: a=${profile.m.a.toStringAsFixed(1)} b=${profile.m.b.toStringAsFixed(1)}');
    print(
        '  Highlight cast: a=${profile.h.a.toStringAsFixed(1)} b=${profile.h.b.toStringAsFixed(1)}');
    ...
    final (mnNew, p95New) = _deltaE(
        chartNew,
        img.copyResize(styleImg,
            width: colorChart.width, height: colorChart.height));
    final (mnOld, p95Old) = _deltaE(
        chartOld,
        img.copyResize(styleImg,
            width: colorChart.width, height: colorChart.height));

    print(
        '  ΔE (chart vs style)  NEW: mean=${mnNew.toStringAsFixed(2)} p95=${p95New.toStringAsFixed(2)}');
    print(
        '  ΔE (chart vs style)  OLD: mean=${mnOld.toStringAsFixed(2)} p95=${p95Old.toStringAsFixed(2)}');
    results.add(
        '$styleName  NEW mean=${mnNew.toStringAsFixed(2)} p95=${p95New.toStringAsFixed(2)}  |  OLD mean=${mnOld.toStringAsFixed(2)} p95=${p95Old.toStringAsFixed(2)}');

    // Save style thumbnail
    final styleThumb = img.copyResize(styleImg,
        width: colorChart.width, height: colorChart.height);

    // Save comparison strips
    final stripChart = _strip(styleName.replaceAll('_', ' '), colorChart,
        chartNew, chartOld, styleThumb);
    final stripGrad = _strip(
        styleName.replaceAll('_', ' '),
        gradientTest,
        gradNew,
        gradOld,
        img.copyResize(styleImg,
            width: gradientTest.width, height: gradientTest.height));
  ...
  File('$outDir/_original_colorchart.png')
      .writeAsBytesSync(img.encodePng(colorChart));
  File('$outDir/_original_gradient.png')
      .writeAsBytesSync(img.encodePng(gradientTest));
